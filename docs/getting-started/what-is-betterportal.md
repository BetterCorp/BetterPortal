...
Each service owns its own code, config, routes, and data. The selected shell service owns navigation, branding, and the user-facing URL. The browser loads page content directly from services through HTMX, so the platform gets microservice isolation without iframe seams or SPA framework complexity.
...
The shell does not proxy page content. Services do not emit absolute URLs. There is no client-side router. Composition happens through real HTML responses, HTMX swaps, and BP route bindings.
...
| App | User-facing portal surface with routes, menu, shell, and hostnames. |
| Shell | Host page, navigation, brand, layout, assets, and fragment mounting. |
