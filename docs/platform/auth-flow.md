...
WorkOS uses provider slugs as keys: WorkOS role `slug` maps to `app.auth.roles[].id`, and WorkOS permission `slug` stores short BP permission keys as `bp_<shortId>_<permission>`. The auth-workos service owns the local `workosStatePath` JSON file that maps each short id back to `tenantId + serviceId + viewId`; roles remain app-scoped when synchronized with BP. BP syncs WorkOS permissions from enabled synced app routes for the tenant, including API routes when they are mounted in app config. Do not subscribe to WorkOS permission webhook events; stale BP permission slugs are deleted when unassigned or renamed `DEL: ...` while still assigned.

`app.auth.roleAuthority` selects role-definition ownership. Auth services advertise supported `provider` and `betterportal` modes through manifest capabilities. Config Manager defaults to `provider` when available; `betterportal` uses the standard BP role editor and pushes BP-owned grants to the provider while preserving unrelated provider permissions.
...
- `bp-routes/login/index.ts` metadata, `GET.ts` login form model, `POST.ts` login submit handler, `_renderer.bootstrap5/GET.tsx` renderer.
...
- `bp-routes/login/_renderer.bootstrap5/_nav.profile.GET.tsx` - nav fragment with user display.
