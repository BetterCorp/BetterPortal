...
For WorkOS-backed apps, Config Manager does not duplicate WorkOS role management. It renders the WorkOS sync fragment in the permission manager, disables local role editing, and mirrors WorkOS role slugs into `app.auth.roles` through the service self-mutation endpoint. BP permissions sync to short WorkOS permission slugs as `bp_<shortId>_<permission>`; the WorkOS service stores the tenant/service/view mapping in its `workosStatePath` JSON file. WorkOS permission webhook events are intentionally ignored to avoid sync loops; role webhook events trigger the WorkOS -> BP mirror.
