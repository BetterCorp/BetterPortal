...
Every theme serves the shared view-authoring rules through `/llms-ui.txt`, followed by links to its own UI guide, skill and templates. `/llms.txt` and the AI manifest also link to `/llms-drop.txt` for tools that need a single full local export. It combines the guides, theme resources and theme schemas with full source URLs, separate framework/theme versions, UTC export time and expiry; remote service schemas remain linked to their authoritative sources. See the [drop contract](../../spec/ai.md#7-full-llm-drop). `/llms-dev.txt` links to these rules too. Keep theme resources consistent with this shared guidance.
