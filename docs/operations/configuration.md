...
There is no `configEncryptionKey` setting. The key that encrypts secret service
config values is generated by the service at install (256-bit, CSPRNG) and kept
in its bootstrap state, so it is deliberately not operator-configurable.
