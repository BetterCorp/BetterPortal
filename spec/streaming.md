...
- `issues` on `error` follows the error shape of `protocol.md` section 4.
...
A streaming view supports multiple representations of the same frame stream, selected by standard content negotiation (`protocol.md` section 3). The handler runs **once per request**; representation only changes encoding and delivery.

| `Accept` | Delivery | Body |
|---|---|---|
| `application/json` | buffered | `{ "items": [...], "summary": ... }` (section 2.1) |
| `application/x-ndjson` | streamed | one frame per line (section 2.2) |
| `text/html` | buffered or streamed | rendered HTML (section 4) |
...
Because nothing is sent until the stream completes, buffered mode uses **real HTTP status codes**: a failure mid-stream returns a standard 4xx/5xx error body per `protocol.md` section 4, not an `error` frame.
...
- Exactly one frame (section 1) per line, newline-terminated.
...
The view's existing SSE convention (`sse.md` section 1, `<route.path>/__sse`) carries the same frames for browser delivery, with frame kinds as SSE event names. See section 4.2 for themed HTML; without a theme context, each event's `data:` is the frame's JSON:
...
When a *view* is slow in distinct regions (dashboard panels) rather than producing a homogeneous item stream, prefer **deferred components** over streaming: each slow region is its own component (own response schema, addressable via `?_c=` per `protocol.md` section 3.4) and the shell lazy-loads it:
...
- MUST serve the buffered JSON representation (section 2.1) for `application/json`, with `jsonResponseSchema` matching the derived shape.
- MUST serve NDJSON frames (section 2.2) for `application/x-ndjson`, terminal frame included.
- MUST validate every frame payload before emission (section 3.1).
- MUST emit frames in legal order (section 1) - items, optional summary, exactly one terminal.
- MUST run the stream in the request that consumes it (section 2.3 - no cross-request stream state).
