...
2. Exposes `GET /.well-known/bp/search` (allowed as a service-specific well-known path per `protocol.md` § 1).
...
| `X-BP-Tenant-Id` | header | tenant scope (see `protocol.md` § 6) |
...
The endpoint is a **streaming view** per `streaming.md`: it MUST support buffered JSON (§ 2.1 there) and SHOULD support NDJSON frames (§ 2.2) so aggregators can consume results as they are found. The item schema is **pinned by this spec** — aggregators need no manifest lookup to validate:
...
- Providers SHOULD answer within 500 ms at p95; aggregators WILL drop slower providers from interactive results (§ 3.3).
...
- Invalid frames from a provider are dropped per `streaming.md` § 3.2 without aborting that provider's stream.
...
- MUST serve `GET /.well-known/bp/search` with CORS per `protocol.md` § 2.
- MUST return `400` for missing/short `q`.
- MUST validate results against the pinned item schema (§ 2.1) before emission.
