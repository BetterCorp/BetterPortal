...
export interface Bootstrap2RouteLink {
  id: string;
  kind?: "page" | "api";
  title: string;
  href: string;
...
export function isUserFacingRoute(route: Pick<Bootstrap2RouteLink, "kind" | "href">): boolean {
  return route.kind !== "api" && !route.href.startsWith("/_bp/");
}

export interface Bootstrap2ShellContext {
  title: string;
  brandName: string;
  logoUrl?: string;
  faviconUrl?: string;
...
export interface Bootstrap2HostPageContext {
  ...
  routeLinks: Bootstrap2RouteLink[];
  serviceOrigins?: Record<string, string>;
  navItems?: Bootstrap2NavItem[];
  loginUrl?: string;
...
export const Bootstrap2Manifest: PluginManifest = createPluginManifest({
  pluginId: "org.betterportal.theme.bootstrap2",
  title: "BetterPortal Bootstrap2 Theme",
...
export interface Bootstrap2NavLeaf {
  kind: "route";
  route: Bootstrap2RouteLink;
  breadcrumb: string;
  label?: string;
}

export interface Bootstrap2NavGroup {
  kind: "group";
  id: string;
  title: string;
  items: Bootstrap2NavItem[];
  active: boolean;
  defaultExpanded?: boolean;
}

export type Bootstrap2NavItem = Bootstrap2NavLeaf | Bootstrap2NavGroup | { kind: "external"; title: string; href: string };
...
function buildNavItems(routeLinks: Bootstrap2RouteLink[]): Bootstrap2NavItem[] {
  const nestedPrefixes = new Set<string>();
  const topLevelRoutes = new Map<string, Bootstrap2RouteLink>();
...
function activeBreadcrumb(navItems: Bootstrap2NavItem[]): string {
...
function renderRouteLink(item: Bootstrap2NavLeaf, dismissMobileMenu = false): HtmlRenderable {
...
export function renderNavItems(navItems: Bootstrap2NavItem[], dismissMobileMenu = false): HtmlRenderable {
...
export function shellStyles(mode: "light" | "dark", themeConfig: BetterPortalThemeConfig) {
  ...
  const rules: Exclude<Parameters<typeof css>[0], string> = {
    ...
    ":root": {
      ...
      "--bp-text": surfaceConfig.text ?? (mode === "dark" ? "#e6edf3" : "#18212a"),
      "--bp-text-soft": surfaceConfig.textSoft ?? (mode === "dark" ? "#8b98a5" : "#66727e"),
      "--bp-border": surfaceConfig.border ?? (mode === "dark" ? "#2a3642" : "#cfd6dc"),
      "--bp-accent": themeConfig.bootstrap.primary ?? "#0f9fa8",
      ...
      "--bs-primary": themeConfig.bootstrap.primary ?? "#0f9fa8",
    ...
    ".bp-shell[data-bp-chrome-full-screen='true'] .bp-admin__sidebar, .bp-shell[data-bp-chrome-full-screen='true'] .bp-admin__topbar, .bp-shell[data-bp-chrome-full-screen='true'] .bp-admin__critical-alerts, .bp-shell[data-bp-chrome-full-screen='true'] .bp-admin__footer": {
      display: "none"
    },
    ...
    ".bp-admin__sidebar": {
      ...
      background: mode === "dark" ? "#121920" : "#202a33",
      border: "none",
    ...
    ".bp-admin__critical-alerts": {
      minWidth: 0
    },
    ".bp-admin__critical-alerts:empty": {
      display: "none"
    },
    ".bp-admin__critical-alerts .alert": {
      margin: 0,
    ...
    /* Bootstrap component defaults for compact operations screens. */
    ...
    ".bp-shell__main .btn:not(.dropdown-toggle), .bp-shell__main .btn:hover, .bp-shell__main .btn:active": {
      boxShadow: "none",
      transform: "none"
    },
    ...
    /* Final flat-surface reset; component colors and state borders stay intact. */
    ".bp-shell .card, .bp-shell .alert, .bp-shell .dropdown-menu, .bp-shell .list-group-item, .bp-shell .modal-content, .bp-shell .toast, .bp-shell .accordion-item, .bp-shell .offcanvas, .bp-shell .popover, .bp-shell .form-control, .bp-shell .form-select, .bp-shell .input-group-text": {
      borderRadius: "2px",
      boxShadow: "none",
      backdropFilter: "none",
      "-webkit-backdrop-filter": "none"
    },
    ".bp-shell .btn, .bp-shell .btn:hover, .bp-shell .btn:active": {
      boxShadow: "none",
      transform: "none"
    },
...
function shellAssetUrl(context: Bootstrap2ShellContext, name: string): string {
  const version = context.assetVersion ? `?v=${encodeURIComponent(context.assetVersion)}` : "";
  return `${context.assetBaseUrl}/${name}${version}`;
}

function Bootstrap2Document(context: Bootstrap2ShellContext): HtmlRenderable {
  return (
...
function buildServiceMap(routeLinks: Bootstrap2RouteLink[]): Record<string, string> {
...
function Bootstrap2LandingBody(context: Bootstrap2HostPageContext): HtmlRenderable {
    ...
    >
      ...
      <div class="bp-admin">
        ...
        <section class="bp-admin__workspace">
          ...
          <div
            class="bp-admin__critical-alerts"
            id="bp-frag-critical-alerts"
            hx-get="/.well-known/bp/shell/fragment/critical-alerts"
            hx-trigger="load, bp:fragments-changed from:body"
            hx-swap="innerHTML"
            data-bp-no-route=""
          ></div>
...
export function renderBootstrap2Shell(context: Bootstrap2ShellContext): string {
  return `<!DOCTYPE html>${Bootstrap2Document(context)}`;
}
...
export function renderBootstrap2HostPage(context: Bootstrap2HostPageContext): string {
  ...
  return renderBootstrap2Shell({
    ...
    bodyHtml: Bootstrap2LandingBody(context)
  });
}

export function renderBootstrap2OfflinePage(assetBaseUrl: string, assetVersion: string): string {
  return renderBootstrap2Shell({
    title: "BetterPortal Offline",
    brandName: "BetterPortal",
    themeMode: "dark",
    ...
    bodyHtml: (
      <div class="bp-shell">
        <div class="bp-admin">
          <aside class="bp-admin__sidebar">{renderBrand("BetterPortal")}</aside>
          <section class="bp-admin__workspace">
            <header class="bp-admin__topbar">
              <div class="bp-admin__topbar-title">Offline</div>
            </header>
            <div class="bp-admin__critical-alerts"></div>
            <section class="bp-admin__content-frame">
              <main class="bp-shell__main">
                <div class="bp-shell__empty-state">
                  <div class="bp-shell__empty-card">
                    <div class="bp-shell__empty-title">No network connection</div>
                    <div class="bp-shell__empty-copy">The Bootstrap2 framework is available. Reconnect to load service content.</div>
                    <div class="bp-shell__empty-actions"><button class="btn btn-primary" type="button" onclick="location.reload()">Retry</button></div>
                  </div>
                </div>
              </main>
            </section>
