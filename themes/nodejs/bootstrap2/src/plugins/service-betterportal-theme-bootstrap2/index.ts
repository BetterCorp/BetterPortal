...
const THEME_CONFIG_SCHEMAS: ConfigSchemaDescriptor[] = [
  {
    id: "theme.bootstrap2.app",
    title: "Theme - Branding & Palette",
    description: "Per-app branding, palette, and mode for the bootstrap2 theme.",
    scope: "app",
    jsonSchema: {
    ...
    fields: [
      ...
      { key: "mode", title: "Default Mode", description: "Theme mode.", scope: "app", visibility: "protected", ownership: "mixed", sourceOfTruth: "bp", required: false, defaultValue: "dark", ui: { control: "select", options: [{ value: "light", label: "Light" }, { value: "dark", label: "Dark" }, { value: "system", label: "System" }] } },
      { key: "primary", title: "Primary Color", description: "Bootstrap primary palette color (hex).", scope: "app", visibility: "protected", ownership: "mixed", sourceOfTruth: "bp", required: false, defaultValue: "#0f9fa8", ui: { control: "color" } },
...
const DEFAULT_BOOTSTRAP2_LOGO_URL = bootstrap2AssetUrl("betterportal-logo.png");
const DEFAULT_BOOTSTRAP2_FAVICON_16_URL = bootstrap2AssetUrl("betterportal-favicon-16.png");
const DEFAULT_BOOTSTRAP2_FAVICON_URL = bootstrap2AssetUrl("betterportal-favicon-32.png");
...
const Config = createConfigSchema(
  {
    name: "service-betterportal-theme-bootstrap2",
...
export class Plugin extends BPService<InstanceType<typeof Config>, typeof EventSchemas> {
  ...
  private requirePortalConfig(): PlatformConfig {
    ...
    if (!cfg) {
      throw new Error(
        "Bootstrap2 theme has no portal config yet - waiting for control-plane sync. Verify the theme is installed and the CP is reachable."
      );
    }
    return cfg;
  }
  ...
  private internalConfigTicket(tenantId: string): ServiceConfigTicketClaims {
    ...
    return {
      ...
      realm: "control-plane",
      tenantId,
      serviceId: "org.betterportal.theme.bootstrap2",
      actions: ["config.read"]
    };
  }
  ...
  private registerRoutes(): void {
    this.app.get("/_themes/bootstrap2/assets/**", (event) => this.handleAsset(event));
  ...
  private handleLlmsDocument(
    ...
    return withObservedEvent(event, this.observability, `theme.bootstrap2.${name}`, (activeEvent) => {
  ...
  private async handleAiManifest(event: BetterPortalEvent): Promise<Response> {
    return withObservedEvent(event, this.observability, "theme.bootstrap2.ai_manifest", (activeEvent, span) => {
      const context = this.resolveThemeAiContext(activeEvent);
      return context
        ? jsonResponse(buildThemeAiManifest(context, Bootstrap2DeveloperResources, span.traceId))
  ...
  private handleLlmsUi(event: BetterPortalEvent): Promise<Response> {
    return this.handleLlmsDocument(event, "llms_ui", (context) => renderThemeLlmsUi(context, Bootstrap2DeveloperResources));
  }

  private async handlePublicDiscovery(event: BetterPortalEvent): Promise<Response> {
    return withObservedEvent(event, this.observability, "theme.bootstrap2.public_discovery", (activeEvent, span) => {
      const context = this.resolveThemeAiContext(activeEvent);
  ...
  private async resolveEffectiveTheme(event: BetterPortalEvent): Promise<{ brandName: string; logoUrl?: string; faviconUrl: string; mode: "light" | "dark"; themeConfig: any; tenantId: string; appId: string } | null> {
    ...
    return {
      ...
      faviconUrl: base.faviconUrl ?? DEFAULT_BOOTSTRAP2_FAVICON_URL,
      mode: effectiveMode,
      themeConfig,
...
<head>
  ...
  <link rel="icon" type="image/png" sizes="16x16" href="${DEFAULT_BOOTSTRAP2_FAVICON_16_URL}">
  <link rel="icon" type="image/png" sizes="32x32" href="${DEFAULT_BOOTSTRAP2_FAVICON_URL}">
  <link rel="stylesheet" href="${bootstrap2AssetUrl("bootstrap.min.css")}">
  <style>
    :root {
...
<body>
  <main class="bp-error-shell">
    <section class="bp-error-card" aria-labelledby="bp-error-title">
      <div class="bp-error-brand">
        <img class="bp-error-logo" src="${DEFAULT_BOOTSTRAP2_LOGO_URL}" alt="" width="32" height="32" loading="eager" decoding="async">
  ...
  private renderConfigUiForm(adminApiBase: string, tenantId: string, appId: string, eff: { brandName: string; documentTitle: string; lightLogoUrl: string; darkLogoUrl: string; faviconUrl: string; mode: string; primary: string; secondary: string; success: string; info: string; warning: string; danger: string }, storedKeys: Set<string>, savedFlash = false): string {
    ...
    const saveUrl = `${adminApiBase.replace(/\/+$/, "")}/apps/${encodeURIComponent(appId)}/theme-config/bootstrap2`;
  ...
  private async computeEffectiveAndStored(tenantId: string, appId: string): Promise<{ eff: any; storedKeys: Set<string>; valid: boolean }> {
    ...
    const eff = {
      ...
      mode: base.mode ?? "dark",
      primary: base.bootstrap.primary ?? "#0f9fa8",
  ...
  private async handleAsset(event: BetterPortalEvent): Promise<Response> {
    return withObservedEvent(event, this.observability, "theme.bootstrap2.asset", async (activeEvent) => {
      const assetPath = activeEvent.url.pathname.replace(/^\/_themes\/bootstrap2\/assets\/?/, "");
      const asset = await loadBootstrap2Asset(assetPath);
      if (!asset) {
        return jsonResponse({
      ...
      return new Response(asset.body, {
        ...
        headers: {
          ...
          "cache-control": assetPath === "bootstrap2-sw.js"
            ? "no-cache"
            : activeEvent.url.searchParams.get("v") === BOOTSTRAP2_VERSION
              ? "public, max-age=31536000, immutable"
              : "no-cache",
          ...(assetPath === "bootstrap2-sw.js" ? { "service-worker-allowed": "/" } : {})
        }
      });
    });
  }
  ...
  private async handleIndex(event: BetterPortalEvent): Promise<Response> {
    return withObservedEvent(event, this.observability, "theme.bootstrap2.index", async (activeEvent) => {
      ...
      return new Response(
        renderBootstrap2HostPage({
          ...
          faviconUrl: baseTheme.faviconUrl ?? DEFAULT_BOOTSTRAP2_FAVICON_URL,
          themeMode: effectiveMode,
          themeConfig: mergedThemeConfig,
          assetBaseUrl: BOOTSTRAP2_ASSET_BASE_URL,
          assetVersion: BOOTSTRAP2_VERSION,
  ...
  private async handleManifest(event: BetterPortalEvent): Promise<Response> {
    return withObservedEvent(event, this.observability, "theme.bootstrap2.manifest", (_activeEvent, span) => {
      return jsonResponse({
        ...this.manifest,
        traceId: span.traceId
      } as JsonValue);
