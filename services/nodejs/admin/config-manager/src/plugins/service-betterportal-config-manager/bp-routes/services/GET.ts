export const operationId = "admin.services.view";
...
const RegisteredServiceItemSchema = av.object({
  ...
  lastSeenAt: av.optional(av.string()).describe("ISO timestamp when the service last authenticated with config-manager."),
  lastSyncAt: av.optional(av.string()).describe("ISO timestamp when config-manager last delivered configuration to the service."),
  syncedVersion: av.optional(av.string()).describe("Latest service manifest version received by config-manager."),
  enabled: av.bool().describe("Whether this service registration is active."),
  scope: av.string().minLength(1).describe("Registration scope: tenant, shared, platform, or shell."),
