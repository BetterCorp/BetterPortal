...
Preview API writes retry revision conflicts up to five total attempts, reloading configuration and rechecking authorization and credential replay each time. Continued contention returns HTTP 503 with `Retry-After: 1`; callers may retry the same request. This also covers preview deletion and expiry cleanup during GET.
