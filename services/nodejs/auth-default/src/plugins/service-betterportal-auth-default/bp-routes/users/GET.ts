...
export const QuerySchema = av.object({ after: av.optional(av.string()) });
export default createHandler({ response: ResponseSchema, query: QuerySchema }, async ctx => {
  ...
  return runtime.identity.storage.transaction(scope, async tx => {
    const page = await tx.page<User>("user", dir, { after: (ctx.query as av.Infer<typeof QuerySchema> | undefined)?.after, limit: 101 });
    const users = page.slice(0, 100);
    const groups = await tx.list("group", dir);
    return {
    nextUrl: page.length > 100 ? ctx.routeUrl?.("users.index", { absolute: true, query: { after: users.at(-1)!.id } }) ?? "" : "",
    ...
    mail: (await tx.list("mail", scope)).filter(m => m.state === "failed").map(m => ({ id: m.id, state: String(m.state), attempts: Number(m.attempts) })),
