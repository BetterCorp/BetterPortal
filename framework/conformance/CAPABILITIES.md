...
Packaged native scaffolding passes 43 CLI/HTTP checks on Windows and Linux:
new-directory safety, identity validation, native builds without Node in PATH,
contract export, locked .NET restores, real config-manager installation,
authenticated JSON/HTML and protected-state restart. Full Bootstrap shell
integration remains pending; these checks use its renderer metadata.
...
| scaffold | codegen/init.ts, cli/bp.ts | Native init commands create standalone installation hosts, discovered authenticated JSON/HTML routes, export factories, BP identity/locks and pinned native package references; existing destinations are rejected | framework/ROUTE-AUTHORING.md; generated READMEs | check_scaffold.py: 43 packaged CLI/HTTP checks on each OS, offline .NET dependency restore, consumer builds with no Node, real CM installation, manifest/readiness, JWT auth and encrypted-state restart |
