...
1,078/1,104 schema probes; token/service security passes 459/459 scenarios and JWKS
...
persistence 70/70 and config HTTP hosting 138/139. The full gate passes 4,258/4,286;
...
| lifecycle | BSB service.ts, bootstrapState.ts | Native ASGI lifespan and ASP.NET hosted service own sync cancellation and service disposal; install/redeem bootstrap and key persistence pending | config.md §2; port READMEs | sync_cases.py: exact-loopback, redirect-denied, shutdown-no-retry, cancelled startup and host lifetime; installation/persisted-key gates pending |
