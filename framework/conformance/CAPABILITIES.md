...
| sync | BSB service.ts connectToControlPlane; scopedConfigCache.ts | Atomic replacement and validated restart cache implemented; standalone manifest POST and SSE/poll transport pending | config.md §2 | snapshot_cases.py: cache-not-ready, failed-persistence, invalid-update and cancellation; transport/bootstrap/reconnect pending |
| readiness | BSB service.ts renderHealth/canReadHealthDiagnostics | Minimal health reflects local/managed readiness; managed snapshot API requires acknowledged manifest flag, and restored cache stays unready; transport proof and authorized diagnostics pending | protocol.md §1.1; port READMEs | hosting_cases.py: public-minimal, missing-snapshot and closed; snapshot_cases.py: first-save failure, managed acknowledgment and cache-not-ready; diagnostic authorization pending |
