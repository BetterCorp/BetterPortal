...
registry checks. Prototype JSON hosts pass 222/222 HTTP/ASGI checks.
...
same upstream null/default defect. The combined gate passes 3,019/3,028. See
...
| multipart/raw | contracts/route.ts, adapters/h3.ts | Starlette/ASP.NET adapters parse bounded forms/uploads with canonical MultipartRequest types, repeat/case preservation and file cleanup; raw/file responses pending | protocol.md; port READMEs | hosting_cases.py: body bounds, repeated/mixed fields, byte arrays and malformed forms; raw-download/bodyless-status pending |
| negotiation | runtime/media.ts, adapters/h3.ts | Native media policy plus JSON/metadata hosts; authorized metadata avoids handler side effects; rendering/streaming integration pending | protocol.md §3; port READMEs | media_cases.py: 124 checks; hosting_cases.py: availability, 406 and metadata; exact-renderer pending |
...
| cors | runtime/h3.ts; BSB service.ts | Native trusted-origin policy integrated into JSON hosts, with per-mount preflights before authentication; rendered/streamed responses pending | protocol.md §2; port READMEs | cors_cases.py: 33 checks; hosting_cases.py: protected preflights, denied methods/origins and auth error headers |
