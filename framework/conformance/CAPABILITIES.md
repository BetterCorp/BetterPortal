...
1,186/1,212 schema probes; token/service security passes 459/459 scenarios and JWKS
...
persistence 70/70, config HTTP hosting 138/139 and protected bootstrap storage 147/147, and installation 131/131. The full gate passes 4,646/4,674;
...
| registration | runtime/handler.ts, generatedRegistry.ts, registry.ts; contracts/registry.ts | Native typed JSON/raw handlers, Operation/Route/Registry, canonical declarations, explicit auth and duplicate/ambiguous-route rejection; full contexts and finite-stream/SSE registration pending | manifest.md §1; port READMEs | handler_cases.py, registry_cases.py, raw_cases.py and check_types.py; per-method-policy, stable-ID, required-auth/schema, duplicate operations and paths |
| manifest | runtime/manifest.ts, registry.ts | Native JSON manifest/discovery generation, optional path variants, dependency aliases/local targets and config admin descriptors and renderer metadata; streaming metadata pending | manifest.md; schema-json.md; port READMEs | registry_cases.py: 89 checks for defaults, identity, discovery, dependency targets, contract binding and schema interchange |
