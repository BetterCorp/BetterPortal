...
internal static class ClientAdapter
{
    public static async Task<object> Run(Node body)
    {
        ...
        try
        {
            ...
            var generated = new ConformanceClients.PeerClient(scope, serviceId: requestId is null ? (string?)body.GetValueOrDefault("serviceId") : null, requestId: (string?)requestId);
            async Task<object?> Call(Node step, CancellationToken cancellation)
            {
                var values = step.GetValueOrDefault("values", new Node { ["params"] = new Node { ["key"] = "item" } });
                return body.GetValueOrDefault("generated") is true
                    ? await generated.CheckGetAsync(System.Text.Json.JsonSerializer.Deserialize<ConformanceClients.PeerClient_CheckGetInputs>(Json.Write(values), Json.Options), cancellation)
                    : await client.RequestAsync((string)step.GetValueOrDefault("operation", "check.get")!, values, cancellation);
            }
            foreach (var step in ((List<object?>)body.GetValueOrDefault("steps", new List<object?> { new Node() })!).Cast<Node>())
                try
                {
                    ...
                    var pending = Call(step, cancellation.Token);
