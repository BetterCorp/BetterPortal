...
public sealed class AppAccess
{
    ...
    public IReadOnlyDictionary<string, string> PermissionAliases()
    {
        ...
        return services.Where(pair => mounted.Contains(pair.Key) && pair.Value.ContainsKey("serviceId"))
    ...
    public bool Allows(Route route, string method, string? path = null, string? fragment = null)
    {
        path ??= route.Paths[0];
        ...
        foreach (var mount in Items(app, "routes"))
        {
            ...
            if (mount["enabled"] is true && services.ContainsKey((string)mount["serviceId"]!) && Equals(mount["viewId"], route.ViewId)
        ...
        if (method == "GET")
        {
            if (!string.IsNullOrEmpty(fragment))
            {
                ...
                if (candidates.Any(item => item["enabled"] is true && services.ContainsKey((string)item["serviceId"]!) && Equals(item["fragmentId"], identifier)
                    && PathMatches((string)item["targetPath"]!, path))) return true;
            }
            if (Items(app, "slots").Any(item => item["enabled"] is true && services.ContainsKey((string)item["serviceId"]!) && Equals(item["viewId"], route.ViewId))) return true;
        }
        return false;
    }
}
