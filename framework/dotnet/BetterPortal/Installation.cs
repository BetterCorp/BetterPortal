...
public sealed class ServiceInstallation : IAsyncDisposable
{
    ...
    private Node Binding(object? value)
    {
        ...
        if (((string)binding["cpUrl"]!).TrimEnd('/') != cp || !Equals(binding["cpJwksUri"], jwks)
            || HttpAddress.Origin(TrustedKeys.SecureEndpoint((string)binding["serviceUrl"]!).AbsoluteUri) != address)
            throw new TokenException("Setup token does not match the configured installation");
        return binding;
    }
    private void Credentials(Node value)
    {
        ...
        {
            var tenant = (Binding(binding).GetValueOrDefault("scope") as Node)?.GetValueOrDefault("tenantId");
    ...
    public async Task<bool> StartAsync(CancellationToken cancellation = default)
    {
        ...
        try
        {
            ...
            if (value.ContainsKey("apiKey")) Credentials(value);
    ...
    private async Task<Node> Redeem(string token, CancellationToken cancellation)
    {
        ...
        using var request = new HttpRequestMessage(HttpMethod.Post, cp + "/.well-known/bp/services/redeem") { Content = JsonContent.Create(payload, options: Json.Options) };
        ...
        if (response.StatusCode != System.Net.HttpStatusCode.OK || !string.Equals(response.Content.Headers.ContentType?.MediaType, "application/json", StringComparison.OrdinalIgnoreCase))
            throw new ArgumentException("Invalid redemption response");
        ...
        {
            if (data.Length + count > 1024 * 1024) throw new ArgumentException("Redemption response exceeds 1 MiB");
            data.Write(buffer, 0, count);
        }
        var value = (Node)Contracts.Parse("ServiceRedeemResponseSchema", Json.Read(Envelope.Utf8.GetString(data.ToArray())))!;
