...
and grants. Delegated mode validates only the service envelope; a host must also
...
This example uses an ephemeral key. Key persistence and remote JWKS caching are
not implemented yet. `Tokens.VerifyAsync` requires a trusted resolver, issuer,
