...
export function renderThemeLlmsDev(context: ThemeLlmsContext): string {
  return [
    ...
    "Handlers use `ctx.routeUrl` for service requests and `ctx.uiRouteUrl` for redirects to mounted GET pages. Renderers use `ctx.url.route` inline for service-owned contextual anchors and service requests. For another service, pass its declared dependency alias as `serviceId`; both helpers resolve it through the synced application route index.",
...
export function renderThemeLlmsUi(context: ThemeLlmsContext, resources: ReadonlyArray<DeveloperResource>): string {
  ...
  const lines = [
    ...
    "Renderers receive `ViewRenderContext` as their second argument: `render(data: ResponseData, ctx: ViewRenderContext): HtmlRenderable`. Use inline JSX `<a href={ctx.url.route(\"archive.call\", { query: { id: c.id } }) ?? undefined}>{c.time}</a>` for a contextual link to an enabled GET page in the owning service. The inline null-to-undefined conversion satisfies JSX href typing; assert required routes resolve. No extra shell attributes or service-local PageLink/router wrapper are needed. The shell maps the service path to the mounted app URL, preserves queries and owns Back/Forward. Do not link to API-only operations.",
    "Use `ctx.url.route()` / `ctx.url.current()` for service requests. Use `ctx.url.uiRoute()` when an app URL is needed directly, including auth links to another service without an owning service wrapper. Handler helpers are `ctx.routeUrl` / `ctx.uiRouteUrl`, not renderer members. Unresolved or ambiguous URL helpers return null; fix required mounts/params rather than masking broken links with a hash or silent span fallback. Do not set a service URL as `hx-push-url`. Use `hx-download` for BP-authenticated files; a native new-tab link does not acquire BP request headers. Use documented `bp-no-override` / `data-bp-no-override` only for intentional native behavior; do not invent router-ignore attributes.",
