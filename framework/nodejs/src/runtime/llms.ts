...
export function renderThemeLlmsDev(context: ThemeLlmsContext): string {
  return [
    ...
    "Handlers use `ctx.routeUrl` for service requests and `ctx.uiRouteUrl` for redirects to mounted GET pages. Renderers use `ctx.url.route` for service requests and inline contextual anchors only to uniquely reversible static same-service page mounts. For another service, pass its declared dependency alias as `serviceId`; cross-service resolution uses the synced application route index; current-service route() resolves from the service registry without checking page mounts.",
...
export function renderThemeLlmsUi(context: ThemeLlmsContext, resources: ReadonlyArray<DeveloperResource>): string {
  ...
  const lines = [
    ...
    "Keep complete page markup in `bp-routes/<route>/_renderer.bootstrap5/GET.tsx`. Shared small components may live in a nearby `components/` directory; do not re-export whole pages from separate view modules. The shell owns primary app navigation and Back controls; content may contain contextual links such as call-table rows, without repeating shell menus or Back links.",
    "Renderers receive `ViewRenderContext` as their second argument: `render(data: ResponseData, ctx: ViewRenderContext): HtmlRenderable`. Use inline JSX `<a href={ctx.url.route(\"archive.call\", { query: { id: c.id } }) ?? undefined}>{c.time}</a>` for a contextual link to a static GET page in the owning service with one unambiguous mount and no service/app path collision. The inline null-to-undefined conversion satisfies JSX href typing; assert required routes resolve. No extra shell attributes or service-local PageLink/router wrapper are needed. The shell maps the service path to the mounted app URL, preserves queries and owns Back/Forward. Do not link to API-only operations.",
    "Use `ctx.url.route()` / `ctx.url.current()` for service requests. Use inline `ctx.url.uiRoute()` for parameterized paths, ambiguous reverse mappings, service/app path collisions, and auth links outside the owning service. The shell matches literal paths/prefixes and takes the first match; it does not expand parameter patterns or reject duplicate reverse mappings. Handler helpers are `ctx.routeUrl` / `ctx.uiRouteUrl`, not renderer members. Current-service route() does not validate whether a view is mounted or uniquely mounted. uiRoute() returns null unless parameters resolve exactly one enabled GET app path; fix ambiguous/missing mounts rather than falling back to route(), a hash or a silent span. Do not set a service URL as `hx-push-url`. Use `hx-download` for BP-authenticated files; a native new-tab link does not acquire BP request headers. Use documented `bp-no-override` / `data-bp-no-override` only for intentional native behavior; do not invent router-ignore attributes.",
    "Route tokens must occupy the whole quoted attribute value, such as `href=\"{archive.call}\"`. Supported attributes are href, action, hx-get, hx-post, hx-put, hx-patch, hx-delete and hx-download; view IDs use letters, digits, underscore, dollar, dot or hyphen. `{archive.call}?id=123` is not interpolation and stays unresolved: use the inline URL helper with query/params. Validate generated service navigation in the real mounted shell, including distinct service/app origins, managed credentials, query preservation and Back/Forward; do not rely only on stubbed URL helpers.",
    "",
