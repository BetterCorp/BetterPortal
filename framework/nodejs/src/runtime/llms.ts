...
export function renderThemeLlmsUi(context: ThemeLlmsContext, resources: ReadonlyArray<DeveloperResource>): string {
  ...
  const lines = [
    ...
    "Preserve HTML5 form validation before every form POST. The backend's input schema is the source of truth: render its browser-expressible constraints as appropriate `type`, `required`, `min`, `max`, `step`, `minlength`, `maxlength` and `pattern` attributes, including constraints in updated fragments. The browser validates the returned HTML controls; it does not automatically execute an AnyVali/JSON schema. Do not invent a second client schema or weaken the server-rendered constraints. Numeric inputs use `min`/`max`/`step`; `pattern` does not validate `type=number`.",
