...
function nodeType(
  ...
  switch (node.kind) {
    ...
    case "array": return `Array<${nodeType(node.items, definitions, resolving)}>`;
    case "tuple": return Array.isArray(node.items)
      ? `[${node.items.map((item) => nodeType(item, definitions, resolving)).join(", ")}]`
      : "unknown";
    case "record": return `Record<string, ${nodeType(node.valueSchema, definitions, resolving)}>`;
    case "optional": return `${nodeType(node.inner, definitions, resolving)} | undefined`;
    case "nullable": return `${nodeType(node.inner, definitions, resolving)} | null`;
    case "union": return Array.isArray(node.variants)
      ? node.variants.map((variant) => nodeType(variant, definitions, resolving)).join(" | ") || "never"
      : "unknown";
    case "intersection": return Array.isArray(node.allOf)
      ? node.allOf.map((schema) => nodeType(schema, definitions, resolving)).join(" & ") || "unknown"
      : "unknown";
    case "ref": {
      const name = typeof node.ref === "string" ? node.ref.replace("#/definitions/", "") : "";
      if (name === "BetterPortalJsonValue") return "JsonValue";
      if (!name || resolving.has(name) || !(name in definitions)) return "unknown";
      const nextResolving = new Set(resolving);
      nextResolving.add(name);
      return nodeType(definitions[name], definitions, nextResolving);
    }
    case "object": {
      ...
      const fields = Object.entries(properties).map(([name, child]) => {
        ...
        return `${JSON.stringify(name)}${optional}: ${nodeType(child, definitions, resolving)}`;
      });
      return `{ ${fields.join("; ")} }`;
    }
...
export function emitTypeScriptClient(_alias: string, contract: BpSchemaOutput): string {
  const lines = [
